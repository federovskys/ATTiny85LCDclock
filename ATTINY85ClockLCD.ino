...
// Pins to ATTINY85: #5 (SDA), #7 (SCK/SCL)
#include <TinyWireM.h> //Library of Wire to support DS1307RTC
// (Real-Time Clock) - Pins to ATTINY85: #5 (SDA), #7 (SCK/SCL)
...
setSyncProvider(RTC.get); // Update the time with data of RTC (Real TimeClock)
...
TinyWireM.endTransmission(); // End Write Transmission
//Clear all digits of displaydisplay.setSegments(data + 10, 1, 0); // Clear digit #1
...
// Plot Minutes - Units
for (k1 = 0; k1 <= 7; k1++) {if (bitRead(mu, k1) == 1) {
